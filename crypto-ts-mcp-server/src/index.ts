 ...
 */
async function fetchCryptoPrice(cryptoId: string): Promise<CryptoPrice | { error: string }> {
  try {
...
server.tool(
  "getCryptoPrice",
  { cryptoId: z.string().describe("The CoinGecko ID of the cryptocurrency (e.g., 'bitcoin', 'ethereum', 'solana')") },
  async ({ cryptoId }) => {
...
server.resource(
  "cryptoPrice",
  new ResourceTemplate("crypto://{cryptoId}/price", { list: undefined }),
  async (uri, params) => {
